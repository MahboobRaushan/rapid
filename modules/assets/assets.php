...
hooks()->add_action('app_init',ASSETS_MODULE_NAME.'_actLib');
function assets_actLib()
{
    $CI = & get_instance();
    $CI->load->library(ASSETS_MODULE_NAME.'/Envapi');
    $envato_res = $CI->envapi->validatePurchase(ASSETS_MODULE_NAME);
    if (!$envato_res) {
...
hooks()->add_action('pre_activate_module', ASSETS_MODULE_NAME.'_sidecheck');
function assets_sidecheck($module_name)
{
    if ($module_name['system_name'] == ASSETS_MODULE_NAME) {
        if (!option_exists(ASSETS_MODULE_NAME.'_verified') && empty(get_option(ASSETS_MODULE_NAME.'_verified')) && !option_exists(ASSETS_MODULE_NAME.'_verification_id') && empty(get_option(ASSETS_MODULE_NAME.'_verification_id'))) {
            $CI = & get_instance();
            $data['submit_url'] = $module_name['system_name'].'/env_ver/activate'; 
            $data['original_url'] = admin_url('modules/activate/'.ASSETS_MODULE_NAME); 
            $data['module_name'] = ASSETS_MODULE_NAME; 
...
hooks()->add_action('pre_deactivate_module', ASSETS_MODULE_NAME.'_deregister');
function assets_deregister($module_name)
{
    if ($module_name['system_name'] == ASSETS_MODULE_NAME) {
        delete_option(ASSETS_MODULE_NAME."_verified");
        delete_option(ASSETS_MODULE_NAME."_verification_id");
        delete_option(ASSETS_MODULE_NAME."_last_verification");
